function Get-DeploySettingsFromDB{
 ...
 $DeployRecord = Invoke-Query -Query "Select top 1 DeploymentCreated, DeployPropertiesJSON from Deploy.Deployment  where json_value(DeployPropertiesJSON,'$.Parameters.dacpacname') = '$DacpacName' order by DeploymentCreated Desc" `
  -Database $Database `
