...
BeforeAll {
    ...
    . $ModulePath\Functions\Internal\Invoke-query.ps1
...
Describe 'Get-DeploySettingsFromDB' {
    It 'should run correct query against the server and make connection to the server specified' {
        ...
        Mock invoke-Query { @{DeploymentCreated=(get-date);DeployPropertiesJSON=""} }

        Get-DeploySettingsFromDB -Server $Server -Database $DBName -User $User -Password $pwd -DacpacName "test"

        Should -invoke invoke-Query -Exactly 1 
        Should -invoke invoke-Query -Exactly 1 -ParameterFilter { $Query -eq "Select top 1 DeploymentCreated, DeployPropertiesJSON from Deploy.Deployment  where json_value(DeployPropertiesJSON,'$.Parameters.dacpacname') = 'test' order by DeploymentCreated Desc" }
        Should -invoke invoke-Query -Exactly 1 -ParameterFilter { $Database -eq $DbName }
        Should -invoke invoke-Query -Exactly 1 -ParameterFilter { $ServerInstance -eq $Server }
    }
    It 'should run and connect with sql auth with correct password' {
        ...
        Should -invoke invoke-Query -Exactly 1 
        Should -invoke invoke-Query -Exactly 1 -ParameterFilter { $Credential.username -eq $user }
        Should -invoke invoke-Query -Exactly 1 -ParameterFilter { $Credential.GetNetworkCredential().Password -eq "insecure" }
    }
    ...
    It 'should run and connect with integrated auth' {
        ...
        Should -invoke invoke-Query -Exactly 1 
        Should -invoke invoke-Query -Exactly 1 -ParameterFilter { $Credential -eq $null }
    }
    ...
    It 'should return Hash from DeployProperties' {
        ...
        Mock invoke-Query { @{DeploymentCreated=(get-date);DeployPropertiesJSON='{"Hash":"1234"}'} }
